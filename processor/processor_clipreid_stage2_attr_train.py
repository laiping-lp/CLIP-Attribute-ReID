# -*- encoding: utf-8 -*-
'''
@File    :   processor_clipreid_stage2_attr_train.py
@Time    :   2025/12/23 20:40:22
@Author  :   laiping
@Version :   1.0
...
def do_train_stage2_attr_train(cfg,
             model,
             center_criterion,
             train_loader_stage2,
             val_loader,
    ...
    for epoch in range(1, epochs + 1):
        ...
        for n_iter, (img, vid, target_cam, target_view,attributes) in enumerate(train_loader_stage2):
            # 初始化时，指定 dtype 为 torch.int64 并与 vid 的类型一致
            gender_per_class = torch.empty_like(vid, dtype=torch.int64, device=vid.device)
            hat_per_class = torch.empty_like(vid, dtype=torch.int64, device=vid.device)
            backpack_per_class = torch.empty_like(vid, dtype=torch.int64, device=vid.device)
            ucc_per_class = torch.empty_like(vid, dtype=torch.int64, device=vid.device)
            ucs_per_class = torch.empty_like(vid, dtype=torch.int64, device=vid.device)
            lcc_per_class = torch.empty_like(vid, dtype=torch.int64, device=vid.device)
            lcs_per_class = torch.empty_like(vid, dtype=torch.int64, device=vid.device)
            for i, attribute in enumerate(attributes):
                gender_per_class[i] = int(attribute['gender'])
                ucc_per_class[i] = int(attribute['upper_color'])
                ucs_per_class[i] = int(attribute['upper_style'])
                lcc_per_class[i] = int(attribute['lower_color'])
                lcs_per_class[i] = int(attribute['lower_style'])
                hat_per_class[i] = int(attribute['hat'])
                backpack_per_class[i] = int(attribute['backpack'])
            optimizer.zero_grad()
            ...
            with amp.autocast(enabled=True):
                score, feat,t_feats,i_feats,text_logits,image_logits = model(x = img, label = target, cam_label=target_cam, view_label=target_view,get_i2t=True,gender_label=gender_per_class,hat_label=hat_per_class,backpack_label= backpack_per_class,ucc_label=ucc_per_class,lcc_label=lcc_per_class,ucs_label=ucs_per_class,lcs_label=lcs_per_class)
                logits = i_feats @ t_feats.t()
                loss = loss_fn(score=score, feat=feat, target = target, target_cam = target_cam, i2tscore = logits,text_features=t_feats,image_features=i_feats,text_logits=text_logits,image_logits=image_logits,pids=vid)
